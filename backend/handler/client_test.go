...
func TestShouldFallbackFromFConversation(t *testing.T) {
	tests := []struct {
		...
		{name: "403 response falls back", err: errors.New("f conversation returned 403: <html><body>blocked</body></html>"), want: true},
		{name: "429 response falls back", err: errors.New("f conversation returned 429: rate limited"), want: true},
		{name: "html response falls back", err: errors.New("f conversation returned 418: <!doctype html><html></html>"), want: true},
